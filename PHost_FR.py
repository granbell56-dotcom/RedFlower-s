...
c = wmi.WMI()
log_file_path = os.path.join(os.getcwd(), "peripheriques_log.txt")
...
def lister_peripheriques():
    ...
    for p in c.Win32_PnPEntity():
        try:
            id_device = p.DeviceID
            vid = "????"
            if "VID_" in id_device:
                vid = id_device.split("VID_")[1][:4].upper()
            marque = vid_dict.get(vid, "Inconnue")

            peripheriques[id_device] = {
                "Nom": p.Name or "Inconnu",
                "Description": p.Description or "Inconnue",
                "Fabricant": getattr(p, "Manufacturer", "Inconnu"),
                "Classe": p.PNPClass or "Non spécifiée",
                "Statut": p.Status or "Inconnu",
                "Caption": p.Caption or "N/A",
                "ID Matériel": p.DeviceID,
                "Compatible ID": getattr(p, "CompatibleID", "Inconnu"),
                "Classe GUID": getattr(p, "ClassGuid", "Inconnu"),
                "Code Erreur": getattr(p, "ConfigManagerErrorCode", "Inconnu"),
                "Emplacement": getattr(p, "LocationInformation", "Inconnu"),
                "Service": getattr(p, "Service", "Inconnu"),
                "Marque": marque
            }
        except Exception:
            continue
...
def main():
    ...
    print("[SCANNE]Surveillance des périphériques démarrée. Presse Ctrl+C pour quitter.")
    anciens = lister_peripheriques()
    try:
        while True:
            time.sleep(0.1)
            actuels = lister_peripheriques()
            nouveaux = set(actuels.keys()) - set(anciens.keys())
            if nouveaux:
                for dev_id in nouveaux:
                    afficher_info(actuels[dev_id])
            anciens = actuels
